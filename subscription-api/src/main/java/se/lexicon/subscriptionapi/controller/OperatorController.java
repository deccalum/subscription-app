...
public class OperatorController {
    ...
    @Operation(summary = "{api.operator.read.summary}", description = "{api.operator.read.description}", security = @SecurityRequirement(name = "bearerAuth"))
    ...
    @PutMapping("/{id}")
    @PreAuthorize("hasRole('ADMIN')")
    @Operation(summary = "{api.operator.update.summary}", description = "{api.operator.update.description}", security = @SecurityRequirement(name = "bearerAuth"))
    public ResponseEntity<OperatorResponse> update(@PathVariable Long id, @Valid @RequestBody OperatorRequest request) {
        return ResponseEntity.ok(operatorService.update(id, request));
    }

    @DeleteMapping("/{id}")
    @PreAuthorize("hasRole('ADMIN')")
    @Operation(summary = "{api.operator.delete.summary}", description = "{api.operator.delete.description}", security = @SecurityRequirement(name = "bearerAuth"))
    ...
    @GetMapping("/search")
    @PreAuthorize("hasAnyRole('USER', 'OPERATOR', 'ADMIN')")
    @Operation(summary = "{api.operator.getName.summary}", description = "{api.operator.getName.description}", security = @SecurityRequirement(name = "bearerAuth"))
    ...
    @PreAuthorize("hasAnyRole('USER', 'OPERATOR', 'ADMIN')")
    public ResponseEntity<OperatorResponse> getNameIgnoreCase(@RequestParam String name) {
        return ResponseEntity.ok(operatorService.getNameIgnoreCase(name));
    }
    ...
    @GetMapping
    @PreAuthorize("hasAnyRole('USER', 'OPERATOR', 'ADMIN')")
    @Operation(summary = "{api.operator.getAll.summary}", description = "{api.operator.getAll.description}", security = @SecurityRequirement(name = "bearerAuth"))
    public ResponseEntity<List<OperatorResponse>> getAll() {
